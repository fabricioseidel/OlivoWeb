...
> - ✅ **Login con Google eliminado** (2026-07-15): el proyecto no usa el servicio, así que se quitó el provider de NextAuth, el botón en login/registro, el enriquecimiento de perfil (avatar/nombre/dirección) y las env vars `GOOGLE_*`.
...
| 4 | ~~Email admin hardcodeado como fallback de OAuth~~ | `src/config/auth.config.ts` | ✅ Resuelto: el fallback ya no existía y el provider de Google (junto con `GOOGLE_ADMIN_EMAILS`) fue eliminado del todo |
