...
export default function SugerenciasPanel({ onAfterDraftsCreated }: Props) {
  ...
  const fetchSuggestions = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/admin/replenishment/suggestions?${params}`
      );
      if (!res.ok) {
  ...
  const generateDrafts = async () => {
    ...
    if (
      !confirm(
        `Se crearán supplier_orders en estado "borrador" para ${withSupplierCount} ítems agrupados por proveedor. ¿Continuar?`
      )
    )
      return;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1 max-w-2xl">
            ...
            <b>{safetyDays}d</b> de safety stock, agrupado por proveedor
            primario.
          </p>
        </div>
        <Link
      ...
      <div className="bg-white ring-1 ring-gray-200 rounded-2xl p-4 sm:p-6 grid grid-cols-2 md:grid-cols-4 gap-4 items-end">
        <NumberField
          label="Ventana ventas (días)"
          value={windowDays}
        ...
        <NumberField
          ...
          value={coverageDays}
          setValue={setCoverageDays}
          min={1}
          max={90}
        />
        <NumberField
          label="Safety stock (días)"
          value={safetyDays}
          setValue={setSafetyDays}
          min={0}
          max={30}
        />
      ...
      {data &&
        data.suppliers.filter((s) => s.supplier_id).length === 0 &&
        !loading && (
          <div className="bg-white ring-1 ring-gray-200 rounded-2xl p-10 text-center text-sm text-gray-500">
              ...
              return (
                <tr key={it.barcode} className="hover:bg-gray-50">
                  ...
                  <td className="text-right px-2 py-2 text-gray-700">
                    {Number(it.velocity_daily).toFixed(2)}
                  </td>
